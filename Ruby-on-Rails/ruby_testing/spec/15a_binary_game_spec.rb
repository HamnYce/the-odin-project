...
describe BinaryGame do
  ...
  describe '#player_input' do
    ...
    context 'when user inputs an incorrect value once, then a valid input' do
      before do
        allow(game_input).to receive(:gets).and_return('a', '8')
      end
    ...
    context 'when user inputs two incorrect values, then a valid input' do
      before do
        allow(game_input).to receive(:gets).and_return('a', '54', '8')
      end
  ...
  describe '#verify_input' do
    ...
    subject(:input_verification) { described_class.new(0, 10) }

    context 'when given a valid input as argument' do
      it 'returns valid input' do
        correct_input = input_verification.verify_input(0, 10, 5)
        expect(correct_input).to_not be_nil
      end

      it 'when given edge input' do
        edge_input = input_verification.verify_input(0, 10, 10)
        expect(edge_input).to_not be_nil
      end
    end

    context 'when given invalid input as argument' do
      it 'returns nil' do
        incorrect_input = input_verification.verify_input(0, 10, 100)
  ...
  describe '#display_turn_order' do
    ...
    let(:binary_guess) { instance_double(BinarySearch) }

    before do
      # You'll need to create a few method stubs.
      allow(binary_guess).to receive(:make_guess)
      allow(binary_guess).to receive(:update_range)
    ...
    it 'increases guess_count by one' do
      expect { game_turn.display_turn_order(binary_guess) }.to change { game_turn.instance_variable_get(:@guess_count) }.by(1)
    end

    # Method with Outgoing Command -> Test that a message is sent
    it 'sends make_guess' do
      expect(binary_guess).to receive(:make_guess)
      game_turn.display_turn_order(binary_guess)
    end

    # Method with Outgoing Command -> Test that a message is sent
    it 'sends update_range' do
      expect(binary_guess).to receive(:update_range)
      game_turn.display_turn_order(binary_guess)
    end
